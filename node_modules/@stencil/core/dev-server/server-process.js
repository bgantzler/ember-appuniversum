/*!
 Stencil Dev Server Process v2.0.3 | MIT Licensed | https://stenciljs.com
 */
'use strict';
...
function sendLogRequest(devServerConfig, req, status, sendMsg) {
...
function serve500(devServerConfig, req, res, error, xSource, sendMsg) {
    try {
...
function serve404(devServerConfig, req, res, xSource, sendMsg) {
    try {
...
function serve404Content(devServerConfig, req, res, content, xSource, sendMsg) {
    try {
...
async function serveFile(devServerConfig, sys, req, res, sendMsg) {
    try {
...
async function serveOpenInEditor(devServerConfig, sys, req, res, sendMsg) {
    let status = 200;
    const data = {};
    try {
...
async function serveDevClient(devServerConfig, sys, req, res, sendMsg) {
    try {
...
let connectorHtml = null;
async function serveDevClientScript(devServerConfig, sys, req, res, sendMsg) {
    try {
...
function serveCompilerRequest(devServerConfig, req, res, sendMsg) {
...
function serveCompilerResponse(devServerConfig, compilerRequestResponse, sendMsg) {
    try {
...
async function serveDirectoryIndex(devServerConfig, sys, req, res, sendMsg) {
    try {
...
function createRequestHandler(devServerConfig, sys, sendMsg) {
    return async function (incomingReq, res) {
        try {
            ...
            if (!isValidUrlBasePath(devServerConfig.basePath, req.url)) {
                ...
                return serve404Content(devServerConfig, req, res, `404 File Not Found, base path: ${devServerConfig.basePath}`, `invalid basePath`, sendMsg);
            }
            try {
                req.stats = await sys.stat(req.filePath);
                if (req.stats.isFile) {
            ...
            serve500(devServerConfig, incomingReq, res, e, `not found error`, sendMsg);
        }
    };
}
...
function createHttpServer(devServerConfig, sys, sendMsg) {
...
var open = async (target, options) => {
	...
	if (options.url) {
		target = new url__default['default'].URL(target).href;
...
function initServerProcess(sendMsg) {
    ...
    let server = null;
    let webSocket = null;
    let sys = index_js.createNodeSys({ process });
